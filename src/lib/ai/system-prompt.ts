...
export function buildSystemPrompt(userPrompt?: string): string {
  const basePrompt = [
    ...
    "The available base tools include bash, web_fetch, and custom_command, and room runs may also expose memory tools plus dedicated per-agent workspace tools.",
...
export function buildRoomBridgePrompt(options?: RoomBridgePromptOptions): string {
  const roomBridgeInstruction = [
    ...
    "In addition to those room tools, you can also use base tools such as bash, web_fetch, and custom_command when they help you inspect the live server environment or gather facts for a better room response.",
