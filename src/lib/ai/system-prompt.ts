...
export function buildSystemPrompt(userPrompt?: string): string {
  const basePrompt = [
    ...
    "Use memory_get for direct handle reads or legacy file-path fallback when you only need an exact saved snippet.",
...
export function buildRoomBridgePrompt(options?: RoomBridgePromptOptions): string {
  const roomBridgeInstruction = [
    ...
    "Use memory_get for direct handle reads and for legacy markdown file-path fallback.",
    ...
    "Use memory_status to inspect which memory backend is active, whether the index is dirty, and whether a rebuild is needed.",
    "Use memory_index when persisted memory changed and you need to refresh or rebuild the memory index before searching again.",
    ...
    "Use send_message_to_room.status to signal whether the room message is pending, streaming, completed, or failed.",
    "If you want one visible Chat Room bubble to grow over time, reuse the same send_message_to_room.messageKey across repeated calls for that one bubble.",
    "For streamed visible room replies, send partial user-facing text with the same messageKey and status=streaming, then end that same bubble with status=completed.",
    "Do not create a brand-new messageKey for every partial rewrite of the same visible room reply; reuse one stable key per streamed bubble.",
    "Only mark send_message_to_room.final as true for the conclusive user-visible message of the current turn.",
    ...
    "Room tools are the only user-visible delivery layer; do not rely on ordinary assistant text to communicate with humans.",
