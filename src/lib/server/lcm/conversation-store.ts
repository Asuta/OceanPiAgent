...
export class ConversationStore {
  ...
  async createConversation(input: CreateConversationInput): Promise<ConversationRecord> {
    const result = this.db.prepare(`INSERT INTO conversations (session_id, session_key, title) VALUES (?, ?, ?)`).run(input.sessionId, input.sessionKey ?? null, input.title ?? null);
  ...
  async getConversationForSession(input: { sessionId?: string; sessionKey?: string }): Promise<ConversationRecord | null> {
  ...
  async getOrCreateConversation(sessionId: string, titleOrOpts?: string | { title?: string; sessionKey?: string }): Promise<ConversationRecord> {
    ...
    if (existing) {
      if (opts.sessionKey && !existing.sessionKey) {
        this.db.prepare(`UPDATE conversations SET session_key = ?, updated_at = datetime('now') WHERE conversation_id = ?`).run(opts.sessionKey, existing.conversationId);
  ...
  async createMessage(input: CreateMessageInput): Promise<MessageRecord> {
    const result = this.db.prepare(
      `INSERT INTO messages (conversation_id, seq, role, content, token_count, created_at)
       VALUES (?, ?, ?, ?, ?, ?)`,
    ).run(input.conversationId, input.seq, input.role, input.content, input.tokenCount ?? estimateTokens(input.content), input.createdAt ?? new Date().toISOString());
  ...
  async hasMessage(conversationId: ConversationId, role: MessageRole, content: string): Promise<boolean> {
    const row = this.db.prepare(
      `SELECT 1 AS count
       FROM messages
  ...
  async countMessagesByIdentity(conversationId: ConversationId, role: MessageRole, content: string): Promise<number> {
    const row = this.db.prepare(
  ...
  async getMessageCount(conversationId: ConversationId): Promise<number> {
    const row = this.db.prepare(`SELECT COUNT(*) AS count FROM messages WHERE conversation_id = ?`).get(conversationId) as unknown as CountRow;
    return row?.count ?? 0;
  }
  ...
  async deleteMessages(messageIds: MessageId[]): Promise<number> {
    ...
    for (const messageId of messageIds) {
      const refRow = this.db.prepare(`SELECT 1 AS found FROM summary_messages WHERE message_id = ? LIMIT 1`).get(messageId) as { found: number } | undefined;
      if (refRow) {
        continue;
      }

      this.db.prepare(`DELETE FROM context_items WHERE item_type = 'message' AND message_id = ?`).run(messageId);
  ...
  private indexMessageForFullText(messageId: MessageId, content: string): void {
    ...
    try {
      this.db.prepare(`INSERT INTO messages_fts(rowid, content) VALUES (?, ?)`).run(messageId, normalizedContent);
    } catch {}
  }
  ...
  private searchFullText(query: string, limit: number, conversationId?: ConversationId, since?: Date, before?: Date): MessageSearchResult[] {
  ...
  private searchLike(query: string, limit: number, conversationId?: ConversationId, since?: Date, before?: Date): MessageSearchResult[] {
    ...
    return rows
      .map((row): MessageSearchResult | null => {
        ...
        if (!plan.terms.every((term) => haystack.includes(term))) {
          return null;
        }
        return {
          messageId: row.message_id,
  ...
  private searchRegex(pattern: string, limit: number, conversationId?: ConversationId, since?: Date, before?: Date): MessageSearchResult[] {
