...
function extractToolCallsFromAssistant(msg: AgentMessageLike): ToolCallLike[] {
  ...
  for (const block of content) {
    ...
    const record = block as { type?: unknown; id?: unknown; call_id?: unknown; name?: unknown };
...
export function sanitizeToolUseResultPairing<T extends AgentMessageLike>(messages: T[]): T[] {
  ...
  for (let i = 0; i < messages.length; i += 1) {
    ...
    if (normalizedAssistant.stopReason === "error" || normalizedAssistant.stopReason === "aborted") {
  ...
  return changed || moved ? out : messages;
}
