...
export function exploreStructuredData(content: string, mimeType?: string, fileName?: string): string {
  ...
  if (extension === "xml" || normalizedMime.startsWith("text/xml") || normalizedMime.startsWith("application/xml")) {
...
function buildTextPrompt(params: { content: string; fileName?: string; mimeType?: string; headers: string[] }): string {
  ...
  return [
    ...
    params.headers.length > 0 ? `Detected section headers: ${params.headers.join(" | ")}` : "Detected section headers: none",
...
async function exploreText(params: ExplorationSummaryInput): Promise<string> {
  ...
  if (params.summarizeText) {
    try {
      const summary = await params.summarizeText(buildTextPrompt({
...
export function extensionFromNameOrMime(fileName?: string, mimeType?: string): string {
  return collectFileNameExtension(fileName) ?? guessMimeExtension(mimeType) ?? "txt";
}
...
export function formatFileReference(input: { fileId: string; fileName?: string; mimeType?: string; byteSize: number; summary: string }): string {
...
export function formatToolOutputReference(input: { fileId: string; toolName?: string; byteSize: number; summary: string }): string {
