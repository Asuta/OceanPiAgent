...
async function fetchJson(pathname, timeoutMs = 5000) {
  return new Promise((resolve, reject) => {
    const req = http.get(`http://127.0.0.1:${chromePort}${pathname}`, { timeout: timeoutMs }, (res) => {
      let data = '';
    ...
    req.on('error', reject);
    req.on('timeout', () => req.destroy(new Error(`Timeout fetching ${pathname}`)));
  });
}

// --- WebSocket 连接管理 ---
let chromePort = process.env.CDP_CHROME_PORT ? parseInt(process.env.CDP_CHROME_PORT, 10) : null;
let chromeWsPath = process.env.CDP_CHROME_WS_PATH || null;
...
async function connect() {
  ...
  let effectiveWsPath = chromeWsPath;
  if (!effectiveWsPath) {
    try {
      const versionInfo = await fetchJson('/json/version');
      if (versionInfo?.webSocketDebuggerUrl) {
        effectiveWsPath = new URL(versionInfo.webSocketDebuggerUrl).pathname;
      }
    } catch {
      try {
        const targets = await fetchJson('/json/list');
        const pageTarget = Array.isArray(targets) ? targets.find((item) => item?.webSocketDebuggerUrl) : null;
        if (pageTarget?.webSocketDebuggerUrl) {
          effectiveWsPath = new URL(pageTarget.webSocketDebuggerUrl).pathname;
        }
      } catch { /* ignore */ }
    }
  }

  const wsUrl = getWebSocketUrl(chromePort, effectiveWsPath);
